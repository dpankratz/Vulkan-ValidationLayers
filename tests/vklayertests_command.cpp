...
TEST_F(VkLayerTest, InvailStorageImageAtmoicOperation) {
    TEST_DESCRIPTION("If storage images use atomic operation, the view's format MUST support VK_FORMAT_FEATURE_STORAGE_IMAGE_ATOMIC_BIT");
    ASSERT_NO_FATAL_FAILURE(Init());
    ASSERT_NO_FATAL_FAILURE(InitRenderTarget());
    ...
    VkFormat format = VK_FORMAT_B8G8R8A8_UNORM; // The format doesn't support VK_FORMAT_FEATURE_STORAGE_IMAGE_ATOMIC_BIT to cause DesiredFailure.
    VkImageObj image(m_device);
    ...
    char const *fsSource =
        ...
        "void main() {\n"
        "      imageAtomicAdd(x, ivec2(0), 1);\n "
        "      imageAtomicAdd(x2[1], ivec2(0), 1);\n "
        "}\n";
    ...
    g_pipe.descriptor_set_->WriteDescriptorImageInfo(1, view, sampler, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_IMAGE_LAYOUT_GENERAL, 2);
